...
namespace timb3
{
    public partial class MainForm : Form
    {
        ...
        private void btnOpenEncrypt_Click(object sender, EventArgs e)
        {
            ...
            {
                decrypted = File.ReadAllBytes(openFileDialog.FileName);
                txtIn.Text = Perevodyatel.BytesToStr(decrypted);
                encrypted = Cipher.Encode(decrypted, (int)numE.Value, r);
                txtOut.Text = Perevodyatel.WordsToStr(encrypted);
            }
        }
        private void btnOpenDecrypt_Click(object sender, EventArgs e)
        {
            ...
            if ((r > 256) && (r < 65535) && openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                txtOut.Text = Perevodyatel.WordsToStr(encrypted);
                //decrypted = Cipher.Decode(encrypted, (int)numE.Value, r);
                decrypted = Cipher.Decode(encrypted, int.Parse(txtD.Text), r); // 
                txtIn.Text =Perevodyatel.BytesToStr(decrypted);
            }
        }
        ...
        {
            if (encrypted != null && saveFileDialog.ShowDialog() == DialogResult.OK)
